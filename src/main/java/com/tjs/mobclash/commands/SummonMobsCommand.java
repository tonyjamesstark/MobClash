...
public class SummonMobsCommand extends BaseCommand {
    ...
    public SummonMobsCommand(MobClashPlugin plugin, SpawnManager spawnManager, LanguageManager langManager) {
    ...
    protected boolean execute(CommandSender sender, String[] args) {
        ...
        ((MobClashPlugin) plugin).log(Level.INFO, "Summoning " + amount + " mob(s) for group '" + 
            groupName + "' in mode '" + mode + "' by " + sender.getName());
        ((MobClashPlugin) plugin).log(Level.INFO, "Available mob types: " + getUniqueMobTypes(spawnEggs));
        
        int spawned = summonMobs(mode, locations, spawnEggs, amount);
        
        ((MobClashPlugin) plugin).log(Level.INFO, "Successfully spawned " + spawned + 
            " mob(s) for group '" + groupName + "'");
    ...
    private int summonMobs(String mode, List<Location> locations, List<EntityType> spawnEggs, int amount) {
        int spawned = 0;
        
        if (mode.equals("random")) {
            Location spawnLoc = locations.get(spawnManager.getRandom().nextInt(locations.size()));
            ((MobClashPlugin) plugin).log(Level.INFO, "Spawning at random location: " +
            ...
            ((MobClashPlugin) plugin).log(Level.INFO, "Spawning at all " + locations.size() + " locations");
