...
public abstract class BaseCommand implements CommandExecutor {
    ...
    public BaseCommand(JavaPlugin plugin, SpawnManager spawnManager, LanguageManager langManager, 
                       String permission, boolean requiresPlayer) {
        this.plugin = plugin;
    ...
    public final boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ((MobClashPlugin) plugin).log(Level.INFO, "Command '" + command.getName() + 
            "' executed by " + sender.getName() + " (type: " + sender.getClass().getSimpleName() + ")");
        
        // Allow command blocks to bypass permission checks
        if (!isCommandBlock(sender)) {
            if (!sender.hasPermission(permission)) {
                ((MobClashPlugin) plugin).log(Level.INFO, "Permission denied for " + 
                    sender.getName() + " - requires: " + permission);
        ...
        if (requiresPlayer && !(sender instanceof Player)) {
            ((MobClashPlugin) plugin).log(Level.INFO, "Command requires player but sender is " + 
